...
var fData = datastore.FileData{
	...
	Version:             1,
	LastModifiedDate:    time.Now().UnixNano() / int64(time.Millisecond),
...
func TestCouchbaseDocumentStore_AppendPatch(t *testing.T) {
	...
	cb.AppendPatch(fData.FileID, patch)
	...
	val, err = cb.GetFileData(fData.FileID)
	require.Nil(t, err, "Failed to correctly retrieve file")
	require.Equal(t, fData.Version, val.Version, "Post-change 1: Returned struct had different file version")
	require.Contains(t, val.Changes, patch.String(), "Post-change 1: Returned struct did not contain patch")
	require.Equal(t, fData.LastModifiedDate, val.LastModifiedDate, "Post-change 1: LastModifiedDate was not updated")
	...
	val, err = cb.GetFileData(fData.FileID)
	require.Nil(t, err, "Failed to correctly retrieve file")
	require.Equal(t, fData.Version, val.Version, "Post-change 2: Returned struct had different file version")
	require.Contains(t, val.Changes, "v2:3:\n7:+5:test2:\n10", "Post-change 2: Returned struct did not contain transformed patch")
	require.Equal(t, fData.LastModifiedDate, val.LastModifiedDate, "Post-change 2: LastModifiedDate was not updated")
	...
	val, err = cb.GetFileData(fData.FileID)
	require.Nil(t, err, "Failed to correctly retrieve file")
	require.Equal(t, fData.Version, val.Version, "Post-change 3: Returned struct had different file version")
	require.Contains(t, val.Changes, patch.String(), "Post-change 3: Returned struct did not contain patch")
	require.Equal(t, fData.LastModifiedDate, val.LastModifiedDate, "Post-change 3: LastModifiedDate was not updated")
}
