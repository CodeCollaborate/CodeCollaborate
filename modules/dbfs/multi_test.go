...
func TestDatabaseImpl_PullFile_MidDelete(t *testing.T) {
	...
	checkPullFile(t, di, file, append(transformedChanges, newChanges[:1]...), string(defaultBaseFile))
	...
	// add change
	appendChangeToFile(t, di, newChanges[1])
	checkPullFile(t, di, file, append(transformedChanges, newChanges[:2]...), string(defaultBaseFile))
	...
	checkPullFile(t, di, file, append(transformedChanges, newChanges[:3]...), string(defaultBaseFile))
	...
	checkPullFile(t, di, file, newChanges[:4], string(newRawFile))
	...
	// add change
	patch, err = patching.NewPatchFromString(newChanges[6])
	require.Nil(t, err)
	appendChangeToFile(t, di, patch.String())
...
func nativeErr(t *testing.T, err error) {
	assert.NoError(t, err, "error in native di.DeleteForScrunching code")
}
